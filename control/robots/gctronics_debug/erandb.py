...
def __write_data(addr, data):
	...
	while 1:
		try:
			...
			trials+=1
			print('RaB I2C failed. Trials=', trials)
			time.sleep(0.1)
			if(trials == 25):
				print('RaB I2C write error occured')
...
def __read_data(addr):
	...
	while 1:
		try:
			...
			time.sleep(0.1)
			if(trials == 25):
				print('RaB I2C read error occured')
				traceback.print_exc(file=sys.stdout)
				sys.exit(1)
...
def e_randb_store_light_conditions():
	try:
		bus.write_byte_data(RANDB_I2C_ADDR, 16, 0)
	except:
		print("light contidtions stored")
		# the board will not respond in time, discard this exception
	#__write_data(16, 0);
	__nop_delay(150000)
...
class ERANDB(object):
    ...
    def __listening(self):
        ...
        while True:
            ...
            if self.transmitData != None:
            	e_randb_send_all_data(self.transmitData)
            	time.sleep(0.01)
            	self.transmit_data = None
