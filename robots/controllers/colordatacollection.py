...
logging.getLogger('erandb').setLevel(50)
...
logging.getLogger('cwe').setLevel(10 if isFaulty else 50)
...
# sclog = Logger('../logs/sc.csv', header, extrafields={'isbyz':isByz, 'isfaulty':isFaulty, 'type':'malicious' if isByz else 'faulty' if isFaulty else 'honest'})

header = ['B','G','R', 'NAME', 'IDX', 'FOOD', 'SUPPORT','STATE']
colorlog = Logger('../logs/color.csv', header, extrafields={'isbyz':isByz, 'isfaulty':isFaulty, 'type':'malicious' if isByz else 'faulty' if isFaulty else 'honest'})
...
cwe = ColorWalkEngine(rwSpeed, [1,1,0.5] if isFaulty else [1,1,1])
...
print("Calibrated colors:", cwe.colors)
print("Calibrated colors bgr:", cwe.ground_truth_bgr)
		...
		if fsm.query(Idle.Start):
			...
			if arrived:
				...
				if tag_id != 0:
					...
					colorlog.log(list(color_to_report)+[color_name_to_report, color_idx_to_report, tag_id, 0, 'scout'])
					cwe.set_leds(0b11111111)
					# voteHash = sendVote(color_to_report, is_useful, vote_support, color_idx_to_report, 0)
					print_color("color: ", [int(a) for a in color_to_report], color_name_to_report, 
								"tagid: ", tag_id, 
...
def STOP(modules = submodules, logs = logmodules):
	...
	# txlog = Logger('../logs/tx.csv', header, extrafields={'isbyz':isByz, 'isfaulty':isFaulty, 'type':'malicious' if isByz else 'faulty' if isFaulty else 'honest'})
