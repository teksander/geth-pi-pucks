		...
		if fsm.query(Idle.IDLE):
			...
			if len(cluster_list) > 0 and not voteHash:
				...
				if len(candidate_cluster) > 0:
					...
					color_to_verify[0] = int(float(cluster[0][0]) / DECIMAL_FACTOR)
					color_to_verify[1] = int(float(cluster[0][1]) / DECIMAL_FACTOR)
					color_to_verify[2] = int(float(cluster[0][2]) / DECIMAL_FACTOR)
					verify_unseen = 1
			...
			if not voteHash:
				...
				arrived, _, color_idx_to_verify = cwe.drive_to_closest_color(color_to_verify, duration=100)
			else:
				arrived =  False
				print("EXIT prepare report process, with non-empty voteHash")
			...
			if arrived:
				...
				if 0<vote_support<address_balance and found_color_idx == color_idx_to_verify:
					...
					if repeat_sampled_color[0]!=-1:
						...
						colorlog.log(list(color_to_verify)+list(color_to_report)+[color_name_to_report, color_idx_to_report, 'verify', found_color_name])
					else:
